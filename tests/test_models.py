...
def test_chat_completion_chunk_handles_role_only_delta() -> None:
    ...
    chunk = ChatCompletionChunk.model_validate({"choices": [{"delta": {"role": "assistant"}}]})
...
def test_model_info_defaults_for_minimal_stub() -> None:
    ...
    assert m.us_provider_available is False
    assert m.eu_focused_provider_available is False


def test_model_info_exposes_trustedrouter_badges() -> None:
    ...
    assert m.us_provider_available is True
    assert m.eu_focused_provider_available is True
